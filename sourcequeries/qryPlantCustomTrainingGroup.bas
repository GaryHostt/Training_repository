...
WHERE (((tblPlantEmployeeQualifications.[Multiple Training].Value) Is Not Null) AND ((tblPlantEmployeeQualifications.[Multiple Employees].Value) Is Not Null) AND ((DateDiff("d",[Date Trained],Date()))<366));
